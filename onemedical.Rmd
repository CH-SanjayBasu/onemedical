...
requirement of E&M visit from any provider 
OM attribution through E&M visit with One Medical
...
clm_sub = clm_dol %>%
  mutate(personid = (`Metaclaims Analytics Medical Person ID`),
         om_flag = 
           ((`Metaclaims Analytics Medical Billing Prov Bill ID`=="460695495")&(`Metaclaims Analytics Medical Billing Prov Npi`==1467701821))|
  ...
  add_count(personid, `Metaclaims Analytics Medical Diag01`) %>%
  group_by(personid) %>%
  filter(any(((`Metaclaims Analytics Medical Procedure Code`>=99201)&(`Metaclaims Analytics Medical Procedure Code`<=99205))|
           (`Metaclaims Analytics Medical Procedure Code`>=99211)&(`Metaclaims Analytics Medical Procedure Code`<=99215))) %>%
  mutate(om_flag = max(om_flag),
         diag1 = `Metaclaims Analytics Medical Diag01`[n == max(n)][1],
         cost = sum(`Metaclaims Analytics Medical Paid Amount Ch`),
         copay = sum(`Metaclaims Analytics Medical Copay Amount`),
         coins = sum(`Metaclaims Analytics Medical Coinsurance Amount`),
         deduct = sum(`Metaclaims Analytics Medical Deductible Amount`)) %>%
  select(personid,om_flag,diag1,cost,copay,coins,deduct) %>%
  distinct()
```
...
rx_sub = rx_dol %>%
  ...
  group_by(personid) %>%
  mutate(cost = sum(`Analytics Claims Pharmacy Paid Amount`),
         copay = sum(`Analytics Claims Pharmacy Copay Amount`),
         coins = sum(`Analytics Claims Pharmacy Coinsurance Amount`),
         deduct = sum(`Analytics Claims Pharmacy Deductible Amount`)) %>%
  select(personid,cost,copay,coins,deduct) %>%
  distinct()
```
