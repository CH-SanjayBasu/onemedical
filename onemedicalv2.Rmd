...
```{r table 2}
matchids = cbind(dta_run$personid.x,dta_run$om_flag) 
colnames(matchids) = c("personid.x","om_flag")
matchids = as.tibble(matchids)

ctr_sub2 = ctr
ctr_sub2$Name = str_to_title(ctr$Name)
ctr_sub2$Name = toTitleCase(ctr$Name)
ctr_sub2$`Primary Diagnosis` = as.character(gsub("[\\.]", "", ctr_sub2$`Primary Diagnosis`))

ctr_sub2 = ctr_sub2 %>%
  separate("Name",c("lastname","empty","firstname"),sep = "([\\, \\ ])", extra="drop", warn = "left") %>%
  left_join(mbr_sub,"firstname","lastname") %>%
  mutate(dos = mdy(DOS),
         `Metaclaims Analytics Medical Service Date Start Date` = dos,
         `Metaclaims Analytics Medical Allowed Amount` = "Allowed",
         om_flag = as.logical(1),
         em_flag = ((CPT=='99201')|
                    (CPT=='99202')|
                    (CPT=='99203')|
...
ctr_sub2$pos[ctr_sub2$mh_flag==1] = "Mental Health and Substance Use" 
ctr_sub2$pos[ctr_sub2$pt_flag==1] = "Physical Medicine" 

feesch_sub = feesch %>%
  mutate(`Metaclaims Analytics Medical Allowed Amount` = Fee) %>%
  select(CPT,`Metaclaims Analytics Medical Allowed Amount`)

ctr_sub2 = full_join(ctr_sub2,feesch_sub,by="CPT") %>%  
  select(-CPT) %>%
  mutate(cnt_flag = 1)

clm_tot2 = bind_rows(clm,ctr_sub2)

tab2 = clm_tot2 %>%
  right_join(matchids, by = c("Metaclaims Analytics Medical Person ID" = "personid.x")) %>%
  mutate(dos = `Metaclaims Analytics Medical Service Date Start Date`) %>%
  group_by("Metaclaims Analytics Medical Person ID",dos) %>%
  mutate(pos = `Metaclaims Analytics Medical Service Category Detail`,
         cost_per_er = (`Metaclaims Analytics Medical Allowed Amount`)*(pos=="Emergency Room"),
         cost_per_hosp = (`Metaclaims Analytics Medical Allowed Amount`)*(pos=="Inpatient Visits"|pos=="Medical"),
         cost_per_pcp = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Office Visits - PCP")|(pos=="Preventive Visits - PCP")),
         cost_per_spec = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Office Visits - Specialist")|(pos=="Preventive Visits - Specialist")),
         cost_per_mh = (`Metaclaims Analytics Medical Allowed Amount`)*(pos=="Mental Health and Substance Use" | pos=="Psychiatry"),
         cost_per_pt = (`Metaclaims Analytics Medical Allowed Amount`)*(pos=="Physical Medicine"),
         cost_per_drugadmin = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Administered drug inc Chemo")|(pos=="Administration of drug")|(pos=="Immunizations")),
         cost_per_surg = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Anesthesia")|(pos=="Outpatient Surgery")|(pos=="Surgery")|(pos=="Surgical and Transplant")),
         cost_per_maternity = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Labor and Delivery" | pos=="Newborns")),
         cost_per_labs = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Lab Pathology" | pos=="Pathology Lab")),
         cost_per_rads = (`Metaclaims Analytics Medical Allowed Amount`)*((pos=="Radiology")),
         cost_per_er = na_if(cost_per_er,0),
         cost_per_hosp = na_if(cost_per_hosp,0),
         cost_per_pcp = na_if(cost_per_pcp,0),
         cost_per_spec = na_if(cost_per_spec,0),
         cost_per_mh = na_if(cost_per_mh,0),
         cost_per_pt = na_if(cost_per_pt,0),
         cost_per_drugadmin = na_if(cost_per_drugadmin,0),
         cost_per_surg = na_if(cost_per_surg,0),
         cost_per_maternity =na_if(cost_per_maternity,0),
         cost_per_labs = na_if(cost_per_labs,0),
         cost_per_rads = na_if(cost_per_rads,0))


tab2tab = CreateTableOne(data =tab2, vars = c("cost_per_er", "cost_per_hosp", "cost_per_pcp","cost_per_spec","cost_per_mh","cost_per_pt" ,"cost_per_drugadmin","cost_per_surg"   ,"cost_per_maternity" ,"cost_per_labs", "cost_per_rads"), strata="om_flag.y", test = F)
print(tab2tab, smd = TRUE, contDigits=1, catDigits=1, noSpaces = TRUE, quote = T)
...
glmMatched11 <- glm(formula = cost_per_er ~ om_flag + age + female + mm + hcc + ccs + zip ,
                   data    = dta_run)

glmMatched11b <- glm(formula = cost_per_hosp ~ om_flag + age + female + mm + hcc + ccs+ zip  ,
                   data    = dta_run)

glmMatched11c <- glm(formula = cost_per_pcp ~ om_flag + age + female + mm + hcc + ccs + zip ,
                   data    = dta_run)


glmMatched5c <- glm(formula = cost_per_spec ~ om_flag + age + female + mm + hcc + ccs + zip ,
                   data    = dta_run)


glmMatched5ca <- glm(formula = cost_per_mh ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run)


glmMatched5cb <- glm(formula = cost_per_pt ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run)



glmMatched5cc <- glm(formula = cost_per_drugadmin ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run)

glmMatched5cd <- glm(formula = cost_per_surg ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run)

glmMatched5ce <- glm(formula = cost_per_maternity ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run)

glmMatched5cf <- glm(formula = cost_per_labs ~ om_flag + age + female + mm + hcc + ccs+ zip  ,
                    data    = dta_run)

glmMatched5cg <- glm(formula = cost_per_rads ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run)
...
glmMatched11_s <- glm(formula = cost_per_er ~ om_flag + age + female + mm + hcc + ccs +zip ,
                    data    = sens_m)

glmMatched11b_s <- glm(formula = cost_per_hosp ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)

glmMatched11c_s <- glm(formula = cost_per_pcp ~ om_flag + age + female + mm + hcc + ccs+zip  ,
                     data    = sens_m)


glmMatched5c_s <- glm(formula = cost_per_spec ~ om_flag + age + female + mm + hcc + ccs+zip  ,
                    data    = sens_m)


glmMatched5ca_s <- glm(formula = cost_per_mh ~ om_flag + age + female + mm + hcc + ccs+zip  ,
                     data    = sens_m)


glmMatched5cb_s <- glm(formula = cost_per_pt ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)



glmMatched5cc_s <- glm(formula = cost_per_drugadmin ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)

glmMatched5cd_s <- glm(formula = cost_per_surg ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)

glmMatched5ce_s <- glm(formula = cost_per_maternity ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)

glmMatched5cf_s <- glm(formula = cost_per_labs ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)

glmMatched5cg_s <- glm(formula = cost_per_rads ~ om_flag + age + female + mm + hcc + ccs +zip ,
                     data    = sens_m)
...
clm_sub2 = clm_dol2 %>%
  mutate(personid = (`Metaclaims Analytics Medical Person ID`),
         female = as.numeric(`Metaclaims Analytics Medical Gender`=="F"),
         firstname = `Metaclaims Analytics Medical First Name`,
         lastname = `Metaclaims Analytics Medical Last Name`,
         pos = `Metaclaims Analytics Medical Service Category Detail`,
         dos = `Metaclaims Analytics Medical Service Date Start Date`,
...
spacex_dat_ana2 = mbr_sub2 %>% 
  full_join(clm_tot2, by = c("firstname","lastname","female")) %>%
  full_join(rx_sub2, by = c("firstname","lastname","female")) %>%
  full_join(vrt_sub, by = c("firstname","lastname")) %>%
  full_join(hcc2, by = c("personid.x" = "HICNO")) %>%
  left_join(ccs, c("diag1")) %>%
  mutate(age = mean(age),
         om_flag = replace_na(om_flag,0),
         vrt_flag = replace_na(vrt_flag,0),
...
spacex_dat_ana2$om_flag = round(spacex_dat_ana2$om_flag)
spacex_dat_cov <- c('age', 'female', 'mm', 'membermo', 'hcc', 'ccs', 'diag1',  'zip')

spacex_dat_nomiss2 = as.data.frame(spacex_dat_ana2)

start_time <- Sys.time()

set.seed(1)
mod_match2 <- matchit(om_flag ~ age + female + ccs + hcc + mm  + zip,
...
posttab2 = print(posttable2, smd = TRUE, contDigits=3, catDigits=3, noSpaces = TRUE, quote = T)

```

```{r etable11}

matchids2 = cbind(dta_run2$personid.x,dta_run$om_flag) 
colnames(matchids2) = c("personid.x","om_flag")
matchids2 = as.tibble(matchids2)

clm_tot2 = bind_rows(clm_cont,ctr_sub2)

tab211 = clm_tot2 %>%
  right_join(matchids2, by = c("Metaclaims Analytics Medical Person ID" = "personid.x")) %>%
  mutate(dos = `Metaclaims Analytics Medical Service Date Start Date`) %>%
  group_by("Metaclaims Analytics Medical Person ID",dos) %>%
  mutate(pos = `Metaclaims Analytics Medical Service Category Detail`,
         cost_per_er = (`Metaclaims Analytics Medical Allowed Amount`)*(pos=="Emergency Room"),
...
tab2tab11 = CreateTableOne(data =tab211, vars = c("cost_per_er", "cost_per_hosp", "cost_per_pcp","cost_per_spec","cost_per_mh","cost_per_pt" ,"cost_per_drugadmin","cost_per_surg"   ,"cost_per_maternity" ,"cost_per_labs", "cost_per_rads"), strata="om_flag.y", test = F)
print(tab2tab11, smd = TRUE, contDigits=1, catDigits=1, noSpaces = TRUE, quote = T)
...
glmMatched11_c <- glm(formula = cost_per_er ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run2)

glmMatched11b_c <- glm(formula = cost_per_hosp ~ om_flag + age + female + mm + hcc + ccs+ zip  ,
                     data    = dta_run2)

glmMatched11c_c <- glm(formula = cost_per_pcp ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)


glmMatched5c_c <- glm(formula = cost_per_spec ~ om_flag + age + female + mm + hcc + ccs + zip ,
                    data    = dta_run2)


glmMatched5ca_c <- glm(formula = cost_per_mh ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)


glmMatched5cb_c <- glm(formula = cost_per_pt ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)



glmMatched5cc_c <- glm(formula = cost_per_drugadmin ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)

glmMatched5cd_c <- glm(formula = cost_per_surg ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)

glmMatched5ce_c <- glm(formula = cost_per_maternity ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)

glmMatched5cf_c <- glm(formula = cost_per_labs ~ om_flag + age + female + mm + hcc + ccs+ zip  ,
                     data    = dta_run2)

glmMatched5cg_c <- glm(formula = cost_per_rads ~ om_flag + age + female + mm + hcc + ccs + zip ,
                     data    = dta_run2)
