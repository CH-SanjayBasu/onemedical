...
clm_sub = clm_dol %>%
  mutate(personid = (`Metaclaims Analytics Medical Person ID`),
         ...
         ctr_flag = as.logical((`Metaclaims Analytics Medical Billing Prov Bill ID`=="800925565")&(`Metaclaims Analytics Medical Billing Prov Npi`==1922470392)),
         near_flag = (ctr_flag!=1)&(om_flag==1),
...
ctr_sub = ctr_sub %>%
  separate("Name",c("lastname","empty","firstname"),sep = "([\\, \\ ])", extra="drop") %>%
  mutate(dos = mdy(DOS)) %>%
  mutate(female= getmode((Gender=='F')),
         ...
         pos = NA,
         ctr_flag=as.logical(1),
         cost_md = as.double("Allowed - Contract"),
         near_flag=as.logical(0))

ctr_sub$pos[ctr_sub$mh_flag==1] = "Mental Health and Substance Use" 
ctr_sub$pos[ctr_sub$pt_flag==1] = "Physical Medicine" 

ctr_sub$pos[ctr_sub$CPT=="10060"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="10061"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="10120"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11100"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11200"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11400"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11401"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11730"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11740"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11900"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="11982"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="17110"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="17111"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="20553"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="20610"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="20612"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="36415"] = "Other"
ctr_sub$pos[ctr_sub$CPT=="69209"] = "Office Visits - PCP"
...
ctr_sub$pos[ctr_sub$CPT=="99243"] = "Office Visits - PCP"
ctr_sub$pos[ctr_sub$CPT=="99244"] = "Office Visits - PCP"
...
ctr_sub$pos[is.na(ctr_sub$pos)==1 & (as.numeric(ctr_sub$CPT)>="10040" & as.numeric(ctr_sub$CPT)<="69210")] = "Office Visits - PCP"
...
ctr_sub = ctr_sub %>%
  mutate(cost_md = cost_md.y) %>%
  select(firstname,lastname,female,em_flag,om_flag,diag1,pos,cost_md,ctr_flag,CPT,near_flag,dos)
...
#  filter(any(em_flag==1)) %>%
  summarise(om_flag = getmode(om_flag[em_flag==1]),
            ctr_flag = getmode(ctr_flag),
            near_flag = getmode(near_flag),
            CPT = getmode(CPT),
            ...
            count_pcp = sum(((pos=="Office Visits - PCP")|(pos=="Preventive Visits - PCP"))&em_flag==1),
            cost_pcp = sum((cost_md[(pos=="Office Visits - PCP"|pos=="Preventive Visits - PCP")&em_flag==1])),
            cost_per_pcp = mean((cost_md[(pos=="Office Visits - PCP"|pos=="Preventive Visits - PCP")&em_flag==1]),na.rm=T),
...
clm_tot = clm_tot  %>%
  full_join(clm_ccs, by = c("firstname","lastname","female"))
...
table(vrt$fce.clinical_encounter_type)/dim(vrt)[1]*100

utilstats = bind_rows(clm_sub,ctr_sub) %>%
  filter(dos>="2017-01-01") %>%
  full_join(mbr_sub, by = c("firstname","lastname","female")) %>%
  full_join(vrt_sub, by = c("firstname","lastname")) 

utilstats$vrt_flag[is.na(utilstats$vrt_flag)==1]=0

mm = mbr_sub %>%
  group_by(firstname,lastname,female) %>%
  summarise(mm = mean(membermo))

sum(na.omit(utilstats$ctr_flag))/sum(mm$mm)*1000
sum(na.omit(utilstats$near_flag))/sum(mm$mm)*1000
sum(na.omit(utilstats$vrt_flag))/sum(mm$mm)*1000

allvis = utilstats %>%
  group_by(firstname,lastname,female) %>%
  summarise(cnt_util =max(ctr_flag)==1,
            cnt_util = replace_na(cnt_util,0),
         near_util = max(near_flag)==1,
            near_util = replace_na(near_util,0),
         vrt_util = max(vrt_flag)==1,
            vrt_util = replace_na(vrt_util,0),
         mm = mean(membermo)) %>%
  select(cnt_util,near_util,vrt_util,mm)

sum(na.omit(allvis$cnt_util==1))/dim(allvis)[1]
sum(na.omit(allvis$near_util==1))/dim(allvis)[1]
sum(na.omit(allvis$vrt_util==1))/dim(allvis)[1]

allvis_df = cbind(allvis$cnt_util,allvis$near_util,allvis$vrt_util)
colnames(allvis_df) = c("On-site visitors", "Near-site visitors", "Virtual visitors")
allvis_df = as.data.frame(allvis_df)
library(vcd)
tbl = structable(`On-site visitors`~`Near-site visitors`+`Virtual visitors`, data =allvis_df)
mosaic(tbl, shade=TRUE, legend=F)
...
clm_hcc  = clm %>%
  filter("Metaclaims Analytics Medical Service Date Start Date"<"2017-01-01") %>%
...
ctr_hcc = ctr_hcc %>%
  ...
  separate("Name",c("lastname","empty","firstname"),sep = "([\\, \\ ])", extra="drop") %>%
  mutate(female= getmode((Gender=='F'))) %>%
  separate(`All Diagnosis`, into=c("diag1","diag2","diag3","diag4","diag5","diag6","diag7","diag8","diag9","diag10"), sep = ", ", extra = "drop") %>%
  full_join(mbr_sub, by = c("firstname","lastname","female"))  %>%
...
mutate(mm = membermo,
       ...
       ccs = as.factor(ccs),
       zip = as.factor(zip),
       ctr_flag = replace_na(ctr_flag,0)) 
...
spacex_dat_nomiss$zip <- substr(spacex_dat_nomiss$zip, 0, 3)

spacex_dat_nomiss = spacex_dat_nomiss[complete.cases(spacex_dat_nomiss),]
#spacex_dat_nomiss = as.data.frame(spacex_dat_ana)

start_time <- Sys.time()

set.seed(1)
pscore_reg = glm(om_flag ~ age + female + ccs + hcc + mm +  zip,
                     family = "binomial", data = spacex_dat_nomiss)
summary(pscore_reg)

mod_match <- matchit(om_flag ~ age + female + ccs + hcc + mm +  zip,
                     method = "nearest", distance = "logit", data = spacex_dat_nomiss, caliper = .10)
end_time <- Sys.time()
...
ctrmtable = CreateTableOne(data =ctrm, vars = c("age", "female", "hcc", "mm",  "cost_md", "cost_rx", "cost_er" ,"cost_hosp" , "cost_pcp"  , "cost_spec"  ,"cost_mh" ,"cost_pt" ,   "cost_drugadmin","cost_surg","cost_maternity","cost_labs","cost_rads","count_er"  , "count_hosp" ,"count_pcp" ,"count_spec", "count_mh",   "count_pt"  , "count_drugadmin","count_surg","count_maternity","count_labs","count_rads", "cost_per_er", "cost_per_hosp", "cost_per_pcp","cost_per_spec","cost_per_mh","cost_per_pt" ,"cost_per_drugadmin","cost_per_surg"   ,"cost_per_maternity" ,"cost_per_labs", "cost_per_rads"), strata="ctr_flag", test = T)
ctrtab = print(ctrmtable, smd = TRUE, contDigits=1, catDigits=1, noSpaces = TRUE, quote = T)

topconds_om = table(spacex_dat_ana$ccs[spacex_dat_ana$om_flag==1 & ((spacex_dat_ana$CPT>= 99210 & spacex_dat_ana$CPT<=99215) | (spacex_dat_ana$CPT>= 99200 & spacex_dat_ana$CPT<=99205))])/length(spacex_dat_ana$ccs[spacex_dat_ana$om_flag==1 & ((spacex_dat_ana$CPT>= 99210 & spacex_dat_ana$CPT<=99215) | (spacex_dat_ana$CPT>= 99200 & spacex_dat_ana$CPT<=99205))])

topconds_con = table(spacex_dat_ana$ccs[spacex_dat_ana$om_flag==0 & ((spacex_dat_ana$CPT>= 99210 & spacex_dat_ana$CPT<=99215) | (spacex_dat_ana$CPT>= 99200 & spacex_dat_ana$CPT<=99205))])/length(spacex_dat_ana$ccs[spacex_dat_ana$om_flag==0 & ((spacex_dat_ana$CPT>= 99210 & spacex_dat_ana$CPT<=99215) | (spacex_dat_ana$CPT>= 99200 & spacex_dat_ana$CPT<=99205))])
...
posttab = print(posttable, smd = TRUE, contDigits=3, catDigits=1, noSpaces = TRUE, quote = T)
...
tempData2 = mice(spacex_dat_ana2, m = 1, maxit = 1, meth = 'cart', seed = 1)
...
spacex_dat_nomiss2 = spacex_dat_nomiss2 %>%
  mutate(diag1=replace_na(diag1,0),
         zip = replace_na(zip,0),
         ccs = replace_na(ccs,0),
         CPT = replace_na(CPT,0))
spacex_dat_nomiss2$zip <- substr(spacex_dat_nomiss2$zip, 0, 3)

#spacex_dat_nomiss2 = spacex_dat_nomiss2[complete.cases(spacex_dat_nomiss2),]
...
pretab2 = print(pretable2, smd = TRUE, contDigits=1, catDigits=1, noSpaces = TRUE, quote = T)
